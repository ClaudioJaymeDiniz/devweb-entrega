...
import { titularesApi, acomodacoesApi, hospedagensApi } from "../lib/api"
...
export default function Dashboard() {
  ...
  const [titularesMap, setTitularesMap] = useState<Record<number, string>>({})
  const [acomodacoesMap, setAcomodacoesMap] = useState<Record<number, string>>({})
  ...
  const loadDashboardData = async () => {
    try {
      const [titulares, acomodacoesData, hospedagensData] = await Promise.all([
        titularesApi.getAll(),
      ...
      const titularesMapData = Object.fromEntries(titulares.map(t => [t.id, t.nome]))
      const acomodacoesMapData = Object.fromEntries(acomodacoesData.map(a => [a.id, a.nome_acomodacao]))
      ...
      setTitularesMap(titularesMapData)
      setAcomodacoesMap(acomodacoesMapData)
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.totalTitulares}</div>
          </CardContent>
        </Card>
                  ...
                  <div key={hospedagem.id} className="flex items-center justify-between border-b pb-2">
                    <div>
                      <p className="font-medium">
                        {titularesMap[hospedagem.cliente_id] || `Titular ID: ${hospedagem.cliente_id}`}
                      </p>
                      <p className="text-sm text-gray-500">
                        {acomodacoesMap[hospedagem.acomodacao_id] || `Acomodação ID: ${hospedagem.acomodacao_id}`}
                      </p>
                    </div>
